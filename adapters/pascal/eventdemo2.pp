...
procedure aqtEventProcessingMode; // Not officially supported...
  cdecl; external;
...
procedure evth(refNum: cint; event: pchar); cdecl;
  begin
    writeln ('cli: ', refNum, 'evt: ', event);
    case decodeEvent(event) of
      0: // Nil event
...
begin
  ...
  while running do
  begin
    writeln('---> enter runLoop');
    aqtEventProcessingMode;
    writeln('---> leaving runLoop');
  end;
